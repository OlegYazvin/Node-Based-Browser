...
- Do not let partial or cancelled installer workflows prune committed installers. Only promote `Installer/` or publish GitHub Release assets after the full installer matrix succeeds for that run.
- The dedicated Linux Mint x64 workflow must publish the refreshed Linux release assets directly to the matching GitHub Release after a successful Mint-compatible build. Do not leave the fixed Mint `.deb` stranded in Actions artifacts while the public release still serves an older broken Linux asset.
