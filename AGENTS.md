...
- After a successful installer or release-asset build, sync the fresh files plus `Installer/manifest.json` and `Installer/README.MD` into the repo snapshot before asking the user to test or calling the installer work done. Do not leave successful GitHub-built installers unstaged in Actions-only artifacts when the user needs them from the repo. If a generated installer exceeds GitHub's hard git file limit, keep it as a GitHub Release asset and still update the repo snapshot for the repo-safe installers so the user has the latest testable files.
